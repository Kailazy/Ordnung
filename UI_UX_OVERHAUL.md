...
| 5 | **Duplicates as node tiles** — each dup fragment of a song as a tile, ergonomic keep/reject, pre-pick the best copy (already does), batch-trash the rejects. | ✅ Done | The stacked `render_copy` rows (Keep\|Delete segmented pairs) are now `render_tile` **node cards** in a `horizontal_wrapped` flow per group — the dupes of one song read as a cluster of tiles. Each 252×104 tile is **click-anywhere-to-toggle** keep⇄reject (the whole card is the target; inner Analyze/Preview/Reveal buttons keep their own clicks via tile-reserved-first ordering + an `inner_clicked` guard). Kept = neutral card w/ green edge + `✓ KEEP` pill; rejected = red-tinted, darker, `🗑 REJECT` pill + struck-out filename; hover brightens the edge. ★ best badge, format+bitrate (right-aligned), and the transcode-quality chip all carry over. Pre-pick best + the `🗑 Delete N marked` batch-trash flow (Trash, recoverable, playlist-slot inheritance, whole-track warning) are unchanged. Builds clean, no new clippy warnings in views.rs. |
