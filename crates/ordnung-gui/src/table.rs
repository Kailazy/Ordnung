...
impl App {
    ...
    pub(crate) fn apply_click_selection(&mut self, id: Id, mods: egui::Modifiers) {
        if mods.command || mods.mac_cmd {
            ...
            let range = anchor.and_then(|a| {
                ...
                Some(self.rows[lo..=hi].iter().map(|r| r.id).collect::<HashSet<_>>())
            });
    ...
    pub(crate) fn save_column_layout(&mut self) {
        self.config.column_order = self.column_order.iter().map(|c| c.key().to_string()).collect();
        self.config.hidden_columns =
            self.hidden_columns.iter().map(|c| c.key().to_string()).collect();
                    ...
                    for (idx, &col) in order.iter().enumerate() {
                            ...
                            .dnd_drag_source(
                                ...
                                |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        // Never let the user hide the last visible
                                        // column — an empty table has no header to
                                        // right-click to reopen this menu.
                                        let enabled = !vis || visible_count > 1;
                                        if ui
                                            .add_enabled(
                                                enabled,
                                                egui::Checkbox::new(&mut vis, col.label()),
                                            )
                                            .changed()
                                        {
                                            if vis {
                        ...
                        if let Some(p) = ui.input(|i| i.pointer.interact_pos()) {
                            if resp.rect.contains(p) {
                                to = Some(if p.y < resp.rect.center().y { idx } else { idx + 1 });
                            }
                        }
                    }
                });
    ...
    pub(crate) fn show_col_filter_popup(&mut self, ctx: &egui::Context) {
        let Some(popup) = self.col_filter_open.take() else { return };
        ...
        egui::Window::new(format!("Filter · {}", col.label()))
            ...
            .show(ctx, |ui| {
                ...
                if col == TableColumn::Quality {
                    ...
                    ui.horizontal(|ui| {
                        if ui
                            ...
                            .on_hover_text("Show only ~320? and lossy copies")
                            .clicked()
                        {
                            text = "~320?, lossy".into();
                            changed = true;
                ...
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new(format!("Filter {}", col.label())).small().weak());
                    let edit = ui.add(
        ...
        if open && !clicked_away {
            self.col_filter_open = Some(ColFilterPopup { col, pos, focus: false });
        }
    }
    ...
    pub(crate) fn draw_table(&mut self, ui: &mut egui::Ui) -> Option<Vec<PathBuf>> {
        ...
        let scroll_out = egui::ScrollArea::horizontal()
            ...
            let mut builder = TableBuilder::new(ui)
                ...
                .id_salt(if show_index { "songs_table_playlist" } else { "songs_table" })
                .striped(true)
                .resizable(true)
                        ...
                        header.col(|ui| {
                            let resp = match col.sort_column() {
                                ...
                                Some(sc) => {
                                    ...
                                    let filt = if filtered_cols.contains(&col) { " ⌕" } else { "" };
                                    let resp = ui.add(
                                        egui::Label::new(
                                            egui::RichText::new(format!("{}{arrow}{filt}", col.label()))
                                                .strong(),
                                        )
                    ...
                    body.rows(ROW_H, self.rows.len(), |mut row| {
                        ...
                        row.col(|ui| {
                            ...
                            let tex = match self.cover_cache.get(&r.id) {
                                Some(ThumbState::Ready(Some(handle))) => Some(handle.clone()),
                                _ => None,
                            };
                            let resp = match tex {
                                Some(handle) => ui.add(
                                    egui::Image::new(&handle)
                                        .fit_to_exact_size(egui::vec2(COVER_PX, COVER_PX))
                                        .sense(egui::Sense::click_and_drag()),
                                ),
                                None => {
                                    if !self.cover_cache.contains_key(&r.id) && has_any_cover {
                            ...
                            let active = matches!(state, PlayState::Loading | PlayState::Playing);
                            let hovering_cover = ui.rect_contains_pointer(resp.rect);
                            if audio_enabled && (hovering_cover || active) {
                                let center = resp.rect.center();
                                let btn = egui::Rect::from_center_size(center, egui::vec2(18.0, 18.0));
                                let pb = ui.interact(
                                    btn,
                                ...
                                let (alpha, glyph, glyph_col) = match state {
                                    ...
                                    PlayState::Loading => (220, "…", egui::Color32::WHITE),
                                    PlayState::Playing => {
                                        (220, "■", egui::Color32::from_rgb(130, 210, 130))
                                    }
                                };
                            ...
                            // row drag-out.
                            if resp.double_clicked() && !over_play_btn { dbl = true; }
                            if resp.drag_started() && !over_play_btn {
                                drag = true;
                                if alt_drag { native_drag_ids = Some(drag_ids.clone()); }
                            }
                        ...
                        row.col(|ui| {
                                ...
                                if col == TableColumn::Key {
                                    if let Some(cam) = r.camelot {
                                        ...
                                        ui.add(egui::Label::new(egui::RichText::new(text).weak()));
                                    }
                                ...
                                if col == TableColumn::Quality {
                                    resp = resp.on_hover_text(quality_tooltip(r));
                                }
                                if resp.clicked() { clicked = true; }
                                if resp.double_clicked() { dbl = true; }
                                ...
                                resp.context_menu(|ui| {
                                    ...
                                    if drag_ids.len() > 1
                                        && ui
                                            .button(format!("Convert {} selected…", drag_ids.len()))
                                            .clicked()
                                    {
                                        menu_action =
                                            Some(TrackMenuAction::ConvertSelected(drag_ids.clone()));
                                        ui.close_menu();
                                    }
                                    ...
                                    ui.menu_button("Add to playlist", |ui| {
                                        ...
                                        for (pid, name) in &menu_playlists {
                                            if ui.button(name).clicked() {
                                                menu_action = Some(TrackMenuAction::AddToPlaylist(
                                                    *pid,
                                                    drag_ids.clone(),
                                                ));
                                                ui.close_menu();
                                    ...
                                    if let Some(pid) = menu_playlist_view {
                                        ...
                                        if ui.button("Remove from playlist").clicked() {
                                            menu_action = Some(TrackMenuAction::RemoveFromPlaylist(
                                                pid,
                                                drag_ids.clone(),
                                            ));
                                            ui.close_menu();
                                        ...
                                        if menu_unfiltered {
                                            ...
                                            if ui.button("Move to bottom").clicked() {
                                                menu_action = Some(TrackMenuAction::MoveToBottom(
                                                    pid,
                                                    drag_ids.clone(),
                                                ));
                                                ui.close_menu();
                                    ...
                                    if ui
                                        .button(slsk_label)
                                        .on_hover_text(soulseek_query(&r.artist, &r.title))
                                        .clicked()
                                    {
                                    ...
                                    if ui
                                        ...
                                        .on_hover_text("Open this track's release on discogs.com")
                                        .clicked()
                                    {
                                    ...
                                    {
                                        menu_action =
                                            Some(TrackMenuAction::FetchArtwork(drag_ids.clone()));
                                        ui.close_menu();
                                    }
                                    ...
                                    {
                                        menu_action =
                                            Some(TrackMenuAction::FetchSongDetails(drag_ids.clone()));
                                        ui.close_menu();
                                    }
                                    ui.separator();
                                    ...
                                    {
                                        menu_action =
                                            Some(TrackMenuAction::DeleteFromCatalog(drag_ids.clone()));
                                        ui.close_menu();
                                    }
                                });
                            });
        ...
        if let Some((col, pos)) = open_col_filter {
            self.col_filter_open = Some(ColFilterPopup { col, pos, focus: true });
        }
...
pub(crate) fn load_rows(db: &Path, filter: &str, view: &LibraryView) -> Result<Vec<TrackRow>, String> {
    ...
    for t in tracks {
        ...
        let (bpm, key) = match analysis.as_ref() {
            Some(a) => (
                a.bpm.map(|b| format!("{b:.0}")).unwrap_or_else(|| "—".into()),
                a.key.map(|k| k.camelot().label()).unwrap_or_else(|| "—".into()),
            ),
            None => ("—".into(), "—".into()),
        };
        ...
        let (duration, bitrate) = match &t.properties {
            Some(p) => (
                ...
                p.bitrate_kbps.map(|b| b.to_string()).unwrap_or_else(|| "—".into()),
            ),
            None => ("—".into(), "—".into()),
        };
        rows.push(TrackRow {
            ...
            added: added_at.get(&t.id).map(|&ts| fmt_added(ts, now)).unwrap_or_default(),
...
pub(crate) fn quality_chip(v: TranscodeVerdict) -> (&'static str, egui::Color32) {
    match v {
        TranscodeVerdict::Clean => ("clean", egui::Color32::from_rgb(70, 130, 80)),
        ...
        TranscodeVerdict::Inconclusive => ("ltd", egui::Color32::from_rgb(90, 95, 105)),
    }
}
...
pub(crate) fn quality_blurb(
    ...
    let mut s = match v {
        TranscodeVerdict::Clean => "Full-band — looks lossless.".to_string(),
        TranscodeVerdict::Suspect => {
        ...
        TranscodeVerdict::Inconclusive => {
            "Band-limited with a gentle roll-off — not a transcode signature.".to_string()
        }
    };
