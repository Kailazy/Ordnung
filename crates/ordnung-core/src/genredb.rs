...
use crate::tools::CountingReader;
...
pub fn import_prebuilt(
    ...
    // The same streaming gunzip the managed ffmpeg download uses.
    let tmp: PathBuf = db_path.with_extension("db.download");
    let completed = crate::tools::download_gunzip(
        PREBUILT_URL,
        "the prebuilt genre database",
        &tmp,
        &mut |p| {
