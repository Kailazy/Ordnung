...
/// Append a decoded buffer to `out` as interleaved frames (L,R,… per frame),
/// preserving every channel — the no-downmix counterpart of [`append_mono`].
fn append_interleaved(buf: &AudioBufferRef, out: &mut Vec<f32>) {
    match buf {
        AudioBufferRef::F32(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f]
        }),
        AudioBufferRef::S16(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f] as f32 / i16::MAX as f32
        }),
        AudioBufferRef::S32(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f] as f32 / i32::MAX as f32
        }),
        AudioBufferRef::U8(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            (b.chan(ch)[f] as f32 - 128.0) / 128.0
        }),
        AudioBufferRef::F64(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f] as f32
        }),
        AudioBufferRef::S24(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f].inner() as f32 / 8_388_607.0
        }),
        AudioBufferRef::U16(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            (b.chan(ch)[f] as f32 - 32_768.0) / 32_768.0
        }),
        AudioBufferRef::U24(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            (b.chan(ch)[f].inner() as f32 - 8_388_608.0) / 8_388_608.0
        }),
        AudioBufferRef::U32(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            (b.chan(ch)[f] as f64 / u32::MAX as f64 * 2.0 - 1.0) as f32
        }),
        AudioBufferRef::S8(b) => interleave(b.spec().channels.count(), b.frames(), out, |ch, f| {
            b.chan(ch)[f] as f32 / i8::MAX as f32
        }),
    }
}

fn interleave<F: Fn(usize, usize) -> f32>(channels: usize, frames: usize, out: &mut Vec<f32>, get: F) {
    out.reserve(frames * channels.max(1));
    for f in 0..frames {
        for ch in 0..channels {
            out.push(get(ch, f));
        }
